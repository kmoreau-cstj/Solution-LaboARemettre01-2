...
{				// Cette accolade ouvre mon bloc d'instruction.
	...
	//FR : Le programme doit ensuite lire l'entier fournis par l'utilisateur pour savoir si c'est un entier pair ou impair 
	std::cin >> entier;

	//Le programme détermine si l'entier est pair ou impair
	if (entier%2 ==0)		// Si notre entier est un multiple de 2 et qu'il n'ya pas de reste, le nombre sera pair. FR : Ma condition est toujours entre parenthèse suivi du if
	{
		std::cout << entier << " est un nombre pair ";			/*Tout texte doit être mis entre "guillemets" et ici, FR : puisque l'entier rempli la conditiom, le programme affiche 
																à l'écran que l'entier est pair*/
	}

	else			//FR : Si l'entier ne rempli la condition, il sera un nombre impair. Pas besoin de répéter la condition puisqu'elle est déjà citée plus haut. 
	{
		std::cout << entier << " est un nombre impair ";		// FR : Puisque l'entier ne rempli pas la condition, le programma affiche à l'écran que l'entier est impair
	}
